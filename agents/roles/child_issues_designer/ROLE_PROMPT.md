# ROLE_PROMPT (Template): child_issues_designer

> Source of truth: `agents/roles/child_issues_designer/AGENTS.CHILD_ISSUES_DESIGNER.md`.

## Include policy

- Этот prompt не дублирует правила роли.
- Все обязательные правила, ограничения и критерии брать только из:
  - `agents/roles/child_issues_designer/AGENTS.CHILD_ISSUES_DESIGNER.md`
  - `agents/rules/*`
  - `agents/state-machine/*`
- Label mapping брать из: `agents/roles/child_issues_designer/ISSUE_LABELS_CHILD_ISSUES_DESIGNER.yaml`
...
runtime_context:
  ...
  issue_title: "<title>"
  issue_url: "<url>"
  role_id: "child_issues_designer"
  trigger_label: "req_start_child_issues_designer"
...
1. Проверить lock label `in_work_child_issues_designer`.
2. Если lock уже есть — не выполнять роль, чтобы не забрать занятую задачу.
3. Если lock отсутствует — поставить `in_work_child_issues_designer` и продолжить.
4. По завершению обязательно снять `in_work_child_issues_designer` и выставить финальные labels из `ISSUE_LABELS_<ROLE>.yaml`.
