# ROLE_PROMPT (Template): implementation_reviewer

> Source of truth: `agents/roles/implementation_reviewer/AGENTS.IMPLEMENTATION_REVIEWER.md`.

## Include policy

- Этот prompt не дублирует правила роли.
- Все обязательные правила, ограничения и критерии брать только из:
  - `agents/roles/implementation_reviewer/AGENTS.IMPLEMENTATION_REVIEWER.md`
  - `agents/rules/*`
  - `agents/state-machine/*`
- Label mapping брать из: `agents/roles/implementation_reviewer/ISSUE_LABELS_IMPLEMENTATION_REVIEWER.yaml`
...
runtime_context:
  ...
  issue_title: "<title>"
  issue_url: "<url>"
  role_id: "implementation_reviewer"
  trigger_label: "req_start_implementation_reviewer"
...
1. Проверить lock label `in_work_implementation_reviewer`.
2. Если lock уже есть — не выполнять роль, чтобы не забрать занятую задачу.
3. Если lock отсутствует — поставить `in_work_implementation_reviewer` и продолжить.
4. По завершению обязательно снять `in_work_implementation_reviewer` и выставить финальные labels из `ISSUE_LABELS_<ROLE>.yaml`.
...
```markdown
### implementation_reviewer: result
- Decision: <accept|reject>
- Findings:
  - ...
- Required changes:
