...
1. Проверить lock label `in_work_solution_architect`.
2. Если lock уже есть — не выполнять роль, чтобы не забрать занятую задачу.
3. Если lock отсутствует — поставить `in_work_solution_architect` и продолжить.
4. По завершению обязательно снять `in_work_solution_architect` и выставить финальные labels из `ISSUE_LABELS_<ROLE>.yaml`.
