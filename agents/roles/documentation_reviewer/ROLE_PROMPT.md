...
1. Проверить lock label `in_work_documentation_reviewer`.
2. Если lock уже есть — не выполнять роль, чтобы не забрать занятую задачу.
3. Если lock отсутствует — поставить `in_work_documentation_reviewer` и продолжить.
4. По завершению обязательно снять `in_work_documentation_reviewer` и выставить финальные labels из `ISSUE_LABELS_<ROLE>.yaml`.
