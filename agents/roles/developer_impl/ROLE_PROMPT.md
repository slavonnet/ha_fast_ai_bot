# ROLE_PROMPT (Template): developer_impl

> Source of truth: `agents/roles/developer_impl/AGENTS.DEVELOPER_IMPL.md`.

## Include policy

- Этот prompt не дублирует правила роли.
- Все обязательные правила, ограничения и критерии брать только из:
  - `agents/roles/developer_impl/AGENTS.DEVELOPER_IMPL.md`
  - `agents/rules/*`
  - `agents/state-machine/*`
- Label mapping брать из: `agents/roles/developer_impl/ISSUE_LABELS_DEVELOPER_IMPL.yaml`
...
runtime_context:
  ...
  issue_title: "<title>"
  issue_url: "<url>"
  role_id: "developer_impl"
  trigger_label: "req_start_developer_impl"
...
1. Проверить lock label `in_work_developer_impl`.
2. Если lock уже есть — не выполнять роль, чтобы не забрать занятую задачу.
3. Если lock отсутствует — поставить `in_work_developer_impl` и продолжить.
4. По завершению обязательно снять `in_work_developer_impl` и выставить финальные labels из `ISSUE_LABELS_<ROLE>.yaml`.
