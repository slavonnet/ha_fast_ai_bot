...
runtime_context:
  ...
  issue_title: "<title>"
  issue_url: "<url>"
  role_id: "orchestrator_story"
  trigger_label: "req_start_orchestrator_story"
...
1. Проверить lock label `in_work_orchestrator_story`.
2. Если lock уже есть — не выполнять роль, чтобы не забрать занятую задачу.
3. Если lock отсутствует — поставить `in_work_orchestrator_story` и продолжить.
4. По завершению обязательно снять `in_work_orchestrator_story` и выставить финальные labels из `ISSUE_LABELS_<ROLE>.yaml`.
